    ...
    <section className="apg-section-divider relative py-24 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-14">
          <p className="text-xs tracking-[0.18em] text-[var(--accent)] uppercase font-extrabold mb-4">{t('why.tag')}</p>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-slate-950">{t('why.title')}</h2>
          <p className="mt-5 text-lg text-slate-700 max-w-3xl mx-auto">{t('why.desc')}</p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {items.map((item, idx) => (
            <motion.div
              ...
              className={[
                'group relative rounded-3xl border border-slate-200 bg-white p-8 text-left shadow-[0_0.875rem_2.5rem_rgba(15,23,42,0.06)]',
                'transition-[transform,box-shadow,border-color,background-color] duration-250 apg-ease',
                'hover:border-slate-300 hover:bg-slate-50 hover:shadow-[0_1.25rem_3.75rem_rgba(15,23,42,0.10)]',
                'focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2',
                pressedIndex === idx ? 'border-slate-300 bg-slate-50 shadow-[0_1.25rem_3.75rem_rgba(15,23,42,0.10)]' : '',
              ].join(' ')}
              role="button"
              tabIndex={0}
              onClick={() => press(idx)}
