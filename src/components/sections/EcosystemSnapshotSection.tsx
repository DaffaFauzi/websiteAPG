      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8 relative">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 sm:mb-24 gap-8">
          <div className="max-w-2xl">
            <motion.div 
              ...
              viewport={{ once: true }}
              className="inline-block px-5 py-2 rounded-full bg-[#0A66C2]/10 text-[#0A66C2] text-[10px] sm:text-xs font-extrabold tracking-[0.2em] uppercase mb-6"
            >
              Strategic Ecosystem
            </motion.div>
            <motion.h2 
              ...
              className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-950 leading-tight tracking-tight"
            >
              {t('ecosystem.title.part1')}
              <span className="text-[#0A66C2] block mt-2">{t('ecosystem.title.part2')}</span>
            </motion.h2>
          </div>
        ...
        <div className="grid gap-6 sm:gap-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            ...
            return (
              <motion.div
                ...
                className={`group relative overflow-hidden rounded-3xl border ${
                  index === 0 
                    ? 'lg:col-span-2 border-[#0A66C2]/20 shadow-lg hover:shadow-xl bg-slate-50' 
                    : 'border-slate-200/60 shadow-sm hover:shadow-md bg-white'
                } p-8 sm:p-10 active:scale-[0.98] transition-all duration-500 apg-ease flex flex-col justify-between min-h-[16rem]`}
              >
                ...
                <div className="absolute -bottom-4 -right-4 h-24 w-24 rounded-full bg-gradient-to-br from-[#0A66C2]/0 to-[#0A66C2]/10 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
              </motion.div>
            );
          })}
          ...
          <motion.div
             ...
             className="group relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#0A66C2] to-[#07337A] p-8 sm:p-10 shadow-[0_20px_40px_rgba(10,102,194,0.3)] flex flex-col justify-center items-center text-center min-h-[16rem] lg:col-span-3"
          >
             <h3 className="text-xl sm:text-2xl font-extrabold text-white mb-4">Explore Full Ecosystem</h3>
             <p className="text-white/80 mb-8 max-w-xs text-sm sm:text-base">Discover how our subsidiaries synergize to create national impact.</p>
             <button className="inline-flex min-h-[3.5rem] items-center justify-center bg-white text-[#0A66C2] font-extrabold px-8 py-4 rounded-full shadow-md hover:shadow-lg hover:bg-slate-50 transition-all duration-300 ease-out hover:-translate-y-[2px] active:translate-y-0 active:scale-[0.98]">
                View Portfolio
             </button>
